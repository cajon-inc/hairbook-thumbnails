...
- 16:54 JST時点の公開フィードsnapshotで、active 972 product / 378 salonを取得。
- 403着地ページを巡回し、動画キャプチャと生成人物を使わず、既存Style／担当者画像937点を選定。
- 同一source・コピー・着地だけを共有化し、964 unique assetを972 productへ割当。
- ヘア、メンズ、ネイル、眉毛・まつ毛、エステを判定し、カテゴリに合わない訴求を出さないコピー規則を適用。
- 1080×1350完成JPGと360×450確認画像を全assetで生成。自動QAは964/964 PASS。
- version付き公開Storageへ1,928ファイルを登録し、Dashboardの事前チェックへ964 asset / 972 assignmentを反映。
- 登録後のライブ在庫は966 productへ変動したため、Dashboardではactive 966 product / 958 unique assetだけを確認対象として表示する。snapshot後に在庫外となった6 productの生成物は版管理上保持するが、公開対象からは自動除外する。
- 人手承認および本番差し替えは0件のまま。全件公開gateは引き続きロック中。
